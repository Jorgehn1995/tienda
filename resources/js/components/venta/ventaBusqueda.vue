<template>
  <div>
          ...
          >
            <template v-slot:item="{ isSelected, toggle, item, index }">
              <v-card
                ...
                :class="
                  seleccionado == index ? 'selected-card' : 'unselected-card'
                "
              ...
              >
                <v-card-text>
                  <v-row dense>
                      ...
                      <v-list dense class="py-0">
                        <v-list-item class="py-0">
                          <v-list-item-content class="py-0">
                            ...
                            <v-list-item-subtitle v-if="item.producto.marca">
                              {{ item.producto.marca }}
                            </v-list-item-subtitle>
                          </v-list-item-content>
                        </v-list-item>
                    ...
                    <v-col cols="12" md="2">
                      <mostrar-precio v-model="item.precio">
                        <template v-slot:moneda>
                          <span class="text-h6 font-weight-bold"> Q </span>
                        </template>
                ...
                <v-card-text class="px-2" v-if="false">
                  <v-row dense style="height: 100%">
                    ...
                    <v-col cols="12" sm="6" md="6">
                      <mostrar-precio v-model="item.precio"></mostrar-precio>
                    </v-col>
                  </v-row>
                </v-card-text>
              </v-card>
        ...
        <v-col cols="12" v-else>
          <v-card elevation="0">
            <v-card-text>
              <div
                class="d-flex justify-center align-center flex-column"
                :style="'height: calc(100vh - ' + getHeight + 'px)'"
              >
                <v-icon size="100" class="grey--text">
                  mdi-tag-search-outline
                </v-icon>
                <br />
                <span> Busca un producto por codigo o por descripción </span>
              </div>
            </v-card-text>
          </v-card>
        </v-col>
    ...
    <div v-shortkey="['ctrl', 'arrowright']" @shortkey="agregarProducto"></div>
    <div v-shortkey="['ctrl', 'arrowleft']" @shortkey="restarProducto"></div>
    <div v-shortkey="['ctrl', 'arrowup']" @shortkey="presentacionArriba"></div>
    <div v-shortkey="['ctrl', 'arrowdown']" @shortkey="presentacionAbajo"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    movimiento() {
      ...
      let card =
        this.$refs["pre" + this.seleccionado].$parent.$parent.$parent.$el
          .offsetTop +
        this.$refs["pre" + this.seleccionado].$parent.$parent.$parent.$el
          .clientHeight;
  ...
  computed: {
    ...
    getHeight() {
      return this.$vuetify.breakpoint.xsOnly ? "320px" : this.height + "px";
    },
  },
  watch: {
    search() {
