  ...
  <div>
    ...
    >
      ...
      <div
        ...
        style="height: 250px"
      >
        <v-icon size="100" class="grey--text"> mdi-cart-variant </v-icon>
        <br />
      ...
      <template v-slot:append>
        <v-card class="mx-2 rounded-lg mb-2" elevation="2">
          <v-list dense color="transparent">
            <v-list-item>
              <v-list-item-subtitle class=""> Subtotal </v-list-item-subtitle>
              <v-list-item-title class="text-right">
                <mostrar-precio v-model="venta.subtotal"></mostrar-precio>
              </v-list-item-title>
            </v-list-item>

            <v-list-item @click="isDiscount = true">
              <v-list-item-subtitle class=""> Descuentos </v-list-item-subtitle>
              <v-list-item-title class="text-right">
                <mostrar-precio v-model="venta.descuento"></mostrar-precio>
              </v-list-item-title>
            </v-list-item>
            <v-divider></v-divider>
            <v-list-item>
    ...
    <v-dialog v-model="isEnded" max-width="400">
      <v-card class="rounded-lg">
        ...
        <v-card-text>
          <v-list dense>
            <v-list-item>
              <v-list-item-subtitle> Articulos </v-list-item-subtitle>
            ...
            <v-list-item>
              <v-list-item-subtitle> Descuento Ofertas </v-list-item-subtitle>
              <v-list-item-title class="text-right grey--text">
                <mostrar-precio v-model="venta.ofertas"></mostrar-precio>
              </v-list-item-title>
            </v-list-item>
            <v-list-item>
              <v-list-item-subtitle> Descuento Manual </v-list-item-subtitle>
              <v-list-item-title class="text-right grey--text">
                <mostrar-precio v-model="venta.descuento"></mostrar-precio>
              </v-list-item-title>
            </v-list-item>
            ...
            <v-list-item>
              ...
              <v-list-item-title class="text-right">
                <div class="d-flex flex-column">
                  <mostrar-precio v-model="venta.total" :size="18">
                  </mostrar-precio>
                </div>
              </v-list-item-title>
            </v-list-item>
            ...
            <v-list-item>
              ...
              <v-list-item-title class="text-right text-h6 grey--text">
                <mostrar-precio v-model="venta.cambio"></mostrar-precio>
              </v-list-item-title>
            </v-list-item>
          </v-list>
          <v-btn
    ...
    <v-dialog v-model="isDone" max-width="400">
      <v-card class="rounded-lg">
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" v-if="false" outlined>Generar Factura</v-btn>
          <v-btn @click="nuevaVenta()" color="primary"> Nueva Venta </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
<script>
import VentaRecibo from "../venta/venta-recibo.vue";
import MostrarPrecio from "../productos/mostrarPrecio.vue";
import VentaBusqueda from "../venta/ventaBusqueda.vue";
import VentaCarrito from "../venta/ventaCarrito.vue";
export default {
  components: { VentaCarrito, VentaBusqueda, MostrarPrecio, VentaRecibo },
  props: {
    caja: {
      type: Object,
      required: true,
  ...
  mounted() {
    let div = document.getElementsByClassName("v-navigation-drawer__content");
    this.venta.idturno = this.turno.idturno;
    this.venta.idcaja = this.caja.idcaja;
  },
  data: () => ({
    drawer: true,
    isLoading: false,
  ...
  methods: {
    ...
    calcularTotales() {
      ...
      this.carrito.forEach((e) => {
        this.venta.articulos = this.venta.articulos + parseInt(e.carrito);
        this.venta.subtotal += parseInt(e.carrito) * parseFloat(e.precio);
        this.venta.costo +=
