      ...
      <div class="top-sticky" v-if="isFound && !isLoading">
        <v-card tile color="primary" dark elevation="0">
          ...
          >
            ...
            <span v-if="!data.nombre"> Producto Sin Registrar </span>
            <span v-else>
              {{ data.nombre }}
              {{ data.marca }}
              {{ data.dimension }}
          ...
          <div v-else class="py-2 pl-1 pr-2 d-flex flex-row align-center">
            <v-btn icon @click="$router.go(-1)" class="mr-2">
              <v-icon> mdi-chevron-left </v-icon>
            </v-btn>
            <span v-if="!data.nombre"> Producto Sin Registrar </span>
            <div v-else>
              <span style="font-family: 'Roboto'; font-size: 1.5em">
                {{ data.nombre }}
              </span>
              <br />
              <span style="font-family: 'Roboto'; font-size: 1.2em">
                {{ data.marca }}
                {{ data.dimension }}
              </span>
            </div>
          ...
          <v-row dense v-if="isFound && !isLoading">
                  ...
                  <v-row dense>
                    <v-col cols="12" sm="6" md="4" class="py-0">
                      <form-text-field
                        v-model="data.codigo"
                        v-shortkey="['ctrl', '1']"
                        @shortkey.native="textfieldEnfocar('codigo')"
                        ref="codigo"
                      ...
                      >
                        <template v-slot:append-outer>
                          <div class="pt-1 pl-1">
                            <v-btn icon @click="codigoGenerar">
            ...
            <v-col cols="12" md="12">
              <v-card outlined elevation="0" class="rounded-lg" height="100%">
                ...
                <v-card-subtitle>
                  Agrega los precios en los que se venderá este producto
                </v-card-subtitle>
            ...
            <v-col cols="12" md="5">
              <v-card outlined elevation="0" class="rounded-lg" height="100%">
                ...
                <v-card-text class="pt-0">
                      ...
                      <template v-slot:default>
                        <thead>
                          <tr>
                            ...
                            <th class="text-center">Costo Nuevo</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr
                            ...
                            @click="
                              (costo = i),
                                textfieldEnfocar('costo' + i, 'corto')
                            "
                          >
                            <td>
                              <v-radio
                                :key="'p' + i"
                                :label="p.nombre || 'Presentacion ' + (i + 1)"
                                :value="i"
                              ></v-radio>
                            </td>
                            <td>
                            ...
                            <td>
                              <div class="pt-1" style="min-width: 120px">
                                <v-text-field
                                  label="Costo Nuevo"
                                  :ref="'costo' + i"
                                  :disabled="costo != i"
                                  ...
                                  type="number"
                                  prefix="Q"
                                  v-model="data.precios[i].costo_nuevo"
                                ></v-text-field>
                              </div>
                            </td>
                          </tr>
                  ...
                  <p>
                    Selecciona la presentacion e indica el costo de la
                    presentación, el resto de costos será calculado de forma
                    automatica
                  </p>
                </v-card-text>
              </v-card>
            ...
            <v-col cols="12" md="7">
              <v-card outlined elevation="0" class="rounded-lg" height="100%">
                ...
                <v-card-text class="pt-0">
                  <v-card class="rounded-lg" color="teal lighten-5 " outlined>
                    <v-list color="transparent">
                      <v-list-item>
                        <v-list-item-subtitle class="teal--text text--darken-3">
                          Existencia Actual
            ...
            <v-col cols="12" md="12">
              <v-card outlined elevation="0" class="rounded-lg">
                ...
                <v-card-subtitle>
                  Visualiza los ultimos 10 ingresos de productos y modifica las
                  fechas de vencimiento de los productos ingresados
                </v-card-subtitle>
                <v-divider></v-divider>
                <v-card-text>
                  <p>
                    Si quieres marcar producto como revisado debes ingresar, al
                    apartado de revisar mercadería
                  </p>
                  <v-btn text outlined>
            ...
            <v-col cols="12">
              <v-row dense>
                <v-col cols="12" sm="6" md="3" order="3" order-sm="1">
                  <v-btn outlined @click="$router.go(-1)" block>
                ...
                <v-col cols="12" sm="12" md="6" order="1" order-sm="3">
                  <v-btn
                    color="primary"
                    dark
                    ...
                    :loading="isSaving"
                  >
                    <v-icon left> mdi-arrow-u-left-bottom </v-icon>
